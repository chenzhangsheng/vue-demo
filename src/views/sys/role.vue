...
<script>
  import panel from 'components/Sys/panel'
  import treeter from 'utils/treeter'
  import { getRoleList,getRoleResource } from 'api/sys';
  ...
  export default {
    ...
    methods: {
      configRoleResources(){
        let checkedKeys = this.$refs.permission_routers.getCheckedKeys();
      ...
      settingResource(event,id){
          ...
          getRoleResource(id).then(response => {
            this.dialogLoading = false;
            this.$refs.addRouters.setCheckedKeys(response.data);
          }).catch(err => {
            this.dialogLoading = false;
            console.log(err);
          });
